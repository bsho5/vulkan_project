...
{   
  ...
  vec3 directionToLight = fragPosWorld-ubo.lightPosition ;

  float attenuation = 1.0 / dot(directionToLight, directionToLight); // distance squared

  vec3 lightColor = ubo.lightColor.xyz * ubo.lightColor.w * (attenuation+0.002);

  vec3 ambientLight = ubo.ambientLightColor.xyz*.01;

  vec3 diffuseLight = lightColor * max(dot(normalize(fragNormalWorld), normalize(directionToLight)), 0);
vec3 lightDir   = normalize(ubo.lightPosition - fragPosWorld);
vec3 viewDir    = normalize(vec3(170.0,-10.0,100.0) - fragPosWorld);
vec3 halfwayDir = normalize(lightDir + viewDir);
float spec = pow(max(dot(fragNormalWorld, halfwayDir), 0.0), .1);
vec3 specular = lightColor * spec;
  color = vec4((fragColor ) * vec3(1.0,1.0,1.0), 1.0);  
