...
namespace lve {
    Texture::Texture(LveDevice &device, const std::string &filepath) : lveDevice{device} {
        ...
        auto data = stbi_load(filepath.c_str(), &width, &height, &m_BytesPerPixel, 0);
...
printf("count: %i \n",12);
        ...
        lveDevice.copyBufferToImage(stagingBuffer.getBuffer(), image, static_cast<uint>(width), static_cast<uint>(height), 1);
